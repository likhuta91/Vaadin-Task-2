...
public class HotelService {
	...
	public synchronized List<Hotel> findAll(String stringFilter, int start, int maxresults) {
		...
		for (Hotel contact : hotels.values()) {
			try {
				boolean passesFilter = passesFilter(contact.toString(), stringFilter); 
				if (passesFilter) {
	...
	public void ensureTestData() {
		if (findAll().isEmpty()) {
			...
			for (String hotel : hotelData) {
				...
				h.setCategory(CategoryService.getInstance().findAll().get(r.nextInt(CategoryService.getInstance().findAll().size())));
				h.setCategory(CategoryService.getInstance().findAll().get(r.nextInt(CategoryService.getInstance().findAll().size())));
				long timeOld = (long)(0 - r.nextDouble() * 30 * 365 * 24 * 60 * 60 * 1000);
				h.setOperatesFrom(new Date().getTime()+timeOld);
				save(h);
			}
		}
	}
